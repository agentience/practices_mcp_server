...
async def get_system_instructions(project_root=None) -> str:
    ...
    if project_root is None:
        ...
        logging.info(f"Detected project root: {project_root}")
    else:
...
def main():
    ...
    logging.info(f"MCP Server starting - Current working directory: {current_dir}")
    
    # Log environment variables to help with debugging
    logging.info(f"Environment variables:")
    for key, value in os.environ.items():
        if "PATH" in key or "HOME" in key or "PWD" in key or "DIR" in key:
    ...
    if args.log_file:
        ...
        if file_handler is None:
            logging.warning("File logging setup failed, continuing with console logging only.")
    else:
